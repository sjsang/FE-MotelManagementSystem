...
export default function ReportFilter({ dateRange, setDateRange, onFilter, onExport, onExportBCA, loading }) {
    ...
    return (
        <div className="card" style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
            ...
            {bcaModalOpen && (
                <div style={{
                    ...
                    <div style={{ width: 400, padding: 24, backgroundColor: '#fff', borderRadius: 12, boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}>
                        <h3 style={{ margin: '0 0 8px 0', fontSize: 18, color: '#1f2a37' }}>🚔 Báo cáo theo quý (TT30/BCA)</h3>
                        ...
                        <div style={{ display: 'flex', gap: 16, marginBottom: 28 }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <select
                                    ...
                                    style={{ width: '100%', padding: '10px 14px', cursor: 'pointer', borderRadius: 8, border: '1px solid #d1d5db' }}
                                >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                            <button
                                ...
                                style={{ padding: '9px 20px', borderRadius: 8, color: '#4b5563', backgroundColor: '#f3f4f6' }}
                            >
                                Hủy
                            </button>
                            <button
                                ...
                                style={{ padding: '9px 24px', borderRadius: 8, background: '#f59e0b', color: '#fff', border: 'none', fontWeight: 600, boxShadow: '0 2px 4px rgba(245,158,11,0.2)' }}
                            >
                                📥 Xuất file
                            </button>
                        </div>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                <div style={{ flex: '0 0 auto', width: 230 }}>
                    <div className="form-label" style={{ fontSize: 13, color: '#9fa3b8', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Kỳ báo cáo</div>
                    <button
                        ref={dateButtonRef}
                        onClick={handleOpenPicker}
                        style={{
                            display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%',
                            padding: '9px 14px', borderRadius: 8, border: '1px solid #C3D6EA',
                            backgroundColor: '#EAF4FF', color: '#1f2a37', cursor: 'pointer',
                            fontFamily: 'inherit', fontSize: 13.5, transition: 'all 0.2s ease',
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                            {dateLabel === 'Chọn trên lịch' ? (
                                <>
                                    ...
                                    <span style={{ margin: '0 10px', color: '#6b7a90', fontWeight: 500 }}>→</span>
                                    <span style={{ fontWeight: 600, letterSpacing: '0.3px' }}>
                                        {dateRange.to ? dayjs(dateRange.to).format('DD/MM/YYYY') : '--'}
                                    </span>
                                </>
                            ) : (
                                <span style={{ fontWeight: 600, letterSpacing: '0.3px' }}>{dateLabel}</span>
                            )}
                        </div>
                    </button>
                    <CustomDateRangePicker
                        open={pickerOpen} anchorEl={pickerAnchor} onClose={() => setPickerOpen(false)}
                        initialDates={{ start: dateRange.from, end: dateRange.to }} onApply={handleApplyDates}
                    />
                </div>

                <button className="btn btn-primary" onClick={onFilter} disabled={loading} style={{ height: 38, padding: '0 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
                <button
                    ...
                    disabled={loading}
                    style={{ height: 38, background: '#f59e0b', color: '#fff', border: 'none', padding: '0 20px', fontWeight: 600 }}
                >
                    Báo cáo theo quý
                </button>

                <button className="btn" onClick={onExport} disabled={loading} style={{ height: 38, background: '#10b981', color: '#fff', border: 'none', padding: '0 20px', fontWeight: 600 }}>
                    Xuất Excel
                </button>
            </div>
        </div>
