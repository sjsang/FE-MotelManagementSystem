...
export default function TabCheckout({
    ...
    discount, setDiscount, taxType, setTaxType, taxInput, setTaxInput,
    previewTotal, taxVnd, payableAmount, deposit, paidAmount
}) {
    return (
        <div>
            {!booking.is_reported && (
            ...
            <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ borderTop: '1px dashed rgba(255,255,255,0.1)', paddingTop: 12, marginTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 16, fontWeight: 700, color: '#60a5fa' }}>{formatCurrency(payableAmount)}</span>
                </div>
                ...
                {deposit > 0 && (
                    <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                            ...
                            <span style={{ fontSize: 13, color: '#10b981' }}> {formatCurrency(deposit)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6, paddingTop: 8, borderTop: '1px dashed rgba(255,255,255,0.1)' }}>
                            ...
                            <span style={{ fontSize: 20, fontWeight: 800, color: '#f59e0b' }}>{formatCurrency(paidAmount)}</span>
                        </div>
                    </>
                )}
            </div>
