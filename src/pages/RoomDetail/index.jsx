...
export default function RoomDetailModal({ room, priceConfig, onClose, onCheckOut, onRefresh, addToast }) {
    ...
    // State quản lý hiển thị Modal xác nhận Check-out
    ...
    const handleCheckOut = async () => {
        setShowConfirmCheckout(false); // Ẩn modal xác nhận
        setLoading(true);
        try {
    ...
    return (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
                    ...
                    {[['info', 'Thông tin'], ['services', `Dịch vụ (${services.length})`], ['checkout', 'Check-out']].map(([key, label]) => (
                        <button key={key} onClick={() => setTab(key)}
                            ...
                            {key === 'services' && saveStatus === 'pending' && (
                                <span style={{ position: 'absolute', top: 8, right: 2, width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', display: 'inline-block' }} />
                            )}
                        </button>
                    ))}
                </div>
            ...
            {/* Modal hiển thị chi tiết hóa đơn sau khi checkout */}
            {createdInvoice && (
                <InvoiceDetailModal
            ...
            {/* Modal Xác nhận Check-out tùy chỉnh */}
            {showConfirmCheckout && (
                <div style={{
                    ...
                    <div style={{
                        background: '#d8f0e5',
                        padding: 24, borderRadius: 12, width: '340px',
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                            <button
                                ...
                                style={{ padding: '8px 14px', borderRadius: 8, background: '#e03d52', color: '#e9f7f6', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>
                                Hủy bỏ
                            </button>
                            <button
                                onClick={handleCheckOut}
                                style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                                Xác nhận
                            </button>
                        </div>
                    </div>
