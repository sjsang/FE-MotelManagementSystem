...
const CustomDateRangePicker = ({ open, anchorEl, onClose, initialDates, onApply }) => {
    ...
    const presets = useMemo(() => {
        ...
        return [
            ...
            { label: 'Năm nay', getDates: () => ({ start: today.startOf('year').format('YYYY-MM-DD'), end: today.endOf('year').format('YYYY-MM-DD') }) },
            { label: 'Năm trước', getDates: () => ({ start: today.subtract(1, 'year').startOf('year').format('YYYY-MM-DD'), end: today.subtract(1, 'year').endOf('year').format('YYYY-MM-DD') }) },
